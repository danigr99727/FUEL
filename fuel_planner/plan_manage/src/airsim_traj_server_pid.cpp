...
void cmdCallback(const ros::TimerEvent& e) {
  ...
  //do frame conversion.. from ned to enu!?
  positionRequest.x = pos(1);
  positionRequest.y = pos(0);
  positionRequest.z = -pos(2);
  positionRequest.yaw = -yaw;
  positionRequest.vehicle_name = "drone_1";
...
void test() {
  ...
  while (tn < duration && ros::ok()) {
    ...
    positionRequest.x = p(1);
    positionRequest.y = p(0);
